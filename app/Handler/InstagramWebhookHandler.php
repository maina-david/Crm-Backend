...
class InstagramWebhookHandler extends ProcessWebhookJob
{
    public function handle()
    {
        ...
        if ($AccountID) {
            $account = InstagramAccount::where('account_id', $AccountID)->first();
            ...
            if ($account) {
                ...
                if ($chatBot) {
                    ...
                    if ($currentConversation) {
                        if ($currentConversation->status == 'CLOSED') {
                            ...
                            $conversation = Conversation::create([
                                ...
                                'customer_id' => $customerID,
                                'channel_id' => 3,
                                'customer_name' => $this->userName($account->facebook_page_id, $customerID),
                                'status' => "ON-BOT",
                            ]);
                            ...
                            while ($continue_loop === true) {
                                ...
                                if ($nextStep->application_type == "SendText") {
                                    ...
                                    $this->sendMessage($customerID, $account->facebook_page_id, $nextStep->application_data);
                                    ...
                                    if ($fileAttachment) {
                                        $this->sendMessage($customerID, $account->facebook_page_id, $fileAttachment->file_url);
                                    } else {
                            ...
                            if ($clientMessage == 'EXIT' || $clientMessage == 'Exit' || $clientMessage == 'exit') {
                                ...
                                $this->sendMessage($customerID, $account->facebook_page_id, 'Goodbye.');
                            } else {
                                ...
                                if (!$selection) {
                                    ...
                                    $this->sendMessage($customerID, $account->facebook_page_id, 'Invalid selection. Try again!');
                                } else {
                            ...
                            $this->sendMessage($customerID, $account->facebook_page_id, 'You are currently on queue. Please wait while we connect you to an available agent.');
                    ...
                    Log::critical("Instagram account not linked to flow!");
                }
            } else {
                Log::critical("Instagram account not found!");
            }
        }
    }
    ...
    /**
     * It takes the AccountID and the customerID and returns the customer's name
     * 
     * @param AccountID The ID of the Facebook Page you want to use.
     * @param customerID The ID of the customer you want to get the name of.
     * 
     * @return The name of the user.
     */
    public function userName($AccountID, $customerID)
    {
        $token = FaceBookPage::find($AccountID);

        $url = "https://graph.facebook.com/$customerID?fields=name,username,profile_pic&access_token=$token->page_access_token";
